{#
    ...
    scd2 / snapshot / time_travel. Run analyses/diagnose_pdm_history.sql to find
    out which of those is actually available to you.
#}

{% set is_backfill = var('report_month') != 'previous_month' %}
{% set mode = var('pdm_history_mode', 'current') %}

{% if is_backfill and mode == 'current' %}

    select
        '{{ var("report_month") }}'     as requested_report_month,
        ...
        || 'Set pdm_history_mode to scd2, snapshot or time_travel.'
                                        as failure_reason
